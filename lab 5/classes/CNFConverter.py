class CNFConverter:
    ...
    # Adjusts the create_P method to handle dictionary input
    def create_P(self, P):
        P1 = {}
        for key in P:
            P1[key] = []
            for production in P[key]:
                # Replaces '' with 'empty' to denote epsilon productions
                if production == '':
                    P1[key].append('empty')
                else:
                    P1[key].append(production)
        return P1
    ...
    def convert_transition(self, trans): 
        ...
        #If it receives more than 2, it separates the first symbol, 
